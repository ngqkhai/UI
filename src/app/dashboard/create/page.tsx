...
import { useScriptWebSocket } from "@/hooks/useScriptWebSocket";
...
export default function CreateVideo() {
  ...
  // Add the WebSocket hook with the stable ref value to avoid reconnections
  const { scriptId, isConnected, status: webSocketStatus } = useScriptWebSocket({
    collectionId: collectionIdRef.current || undefined,
    onScriptGenerated: async (generatedScriptId) => {
      try {
        // Fetch the generated script using the script ID
        const scriptResponse = await fetch(API_ENDPOINTS.script(generatedScriptId));
        if (scriptResponse.ok) {
          ...
          // Redirect to the script review page with the script ID
          router.push(`/dashboard/script-review/${generatedScriptId}`);
        } else {
          throw new Error("Failed to fetch generated script");
        }
      } catch (error) {
  ...
  const debugWebSocket = () => {
    if (collectionIdRef.current) {
      console.log("Debugging WebSocket for collection ID:", collectionIdRef.current);
      
      // Log WebSocket status
      console.log("Current WebSocket status:", webSocketStatus);
      console.log("Is connected:", isConnected);
      console.log("Script ID:", scriptId);
      console.log("Collection ID state:", currentCollectionId);
      console.log("Collection ID ref:", collectionIdRef.current);
      
      // Manually send a test message to verify the connection
      try {
        const socket = WebSocketManager.getConnection(collectionIdRef.current, 
          process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:8002/direct-ws');
      ...
      // Add debug button to show active connections
      WebSocketManager.debugConnections();
      
      // Log recent messages
      console.log("Try triggering script generation again for collection:", collectionIdRef.current);
    } else {
      console.log("No active collection ID to debug");
    }
  };
